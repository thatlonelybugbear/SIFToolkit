export function loadUtils(){
    let utils = {
        patchItemSheet: async function (args){
            ...
            let itemData = app.object.embeddedObject?.getFlag("siftoolkit","SIFData")??app.object.getFlag("siftoolkit","SIFData");
            ...
            let isArea = (app.object.data?.data?.area?.areaType != "" && app.object.data?.data?.area?.areaType);
            ...
            if(app.object.isEmbedded && ["consumable"].includes(app.object.type)){
                renderedTemplate = await renderTemplate("./modules/siftoolkit/templates/embedded_itemSheet_pf2e.handlebars",{});
            }else{
                renderedTemplate = await renderTemplate("./modules/siftoolkit/templates/itemSheet_pf2e.handlebars",
                    {
                        isArea:isArea,
                        isCone:isCone,
            ...
            if(app.object.isEmbedded && ["consumable"].includes(app.object.type)){
                if(itemData == undefined || Object.keys(itemData).length === 0){
                    ...
                    if(app.isEditable){
                        app.object.setFlag("siftoolkit","SIFData",itemData);                    
                    }                    
                }
            }else{
                if(itemData == undefined || Object.keys(itemData).length === 0){
